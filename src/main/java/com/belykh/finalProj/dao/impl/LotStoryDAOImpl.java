...
public class LotStoryDAOImpl implements LotStoryDAO {
    ...
    private static final String SQL_FIND_LOTSTORY_BY_ID="SELECT `lot_story`.`id`,`lot_story`.`user_id_fk`,`lot_story`.lot_id_fk FROM `lot_story` WHERE `lot_story`.`id`=?";
    private static final String SQL_FIND_LOTSTORY_BY_USER_ID = "SELECT `lot_story`.`id`,`lot_story`.`user_id_fk`,`lot_story`.lot_id_fk FROM `lot_story` WHERE `lot_story`.`user_id_fk`=?";
    private static final String SQL_FIND_LOTSTORY_BY_LOT_ID = "SELECT `lot_story`.`id`,`lot_story`.`user_id_fk`,`lot_story`.lot_id_fk FROM `lot_story` WHERE `lot_story`.`lot_id_fk`=?";
    private static final String SQL_ADD_LOTSTORY = "INSERT INTO `lot_story` (`user_id_fk`, `lot_id_fk`) VALUES (?,?)";
    ...
    private LotStoryDBO createLotStory(ResultSet resultSet) throws SQLException {
        ...
        return new LotStoryDBO(id,userId,lotId);
    }
